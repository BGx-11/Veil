...
function Toggle({ on, loading, onClick }: { on: boolean; loading: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`relative w-11 h-6 rounded-full transition-all duration-300 flex-shrink-0 ${on ? 'bg-[var(--accent-primary)] shadow-inner' : 'bg-slate-200/60 shadow-inner'}`}
...
export default function Settings({ settings, onToggle }: SettingsProps) {
  ...
  const handleClearHistory = () => {
    if (confirm('Are you sure you want to clear your browsing history?')) {
      useBrowserStore.getState().setGlobalHistory([]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="glass-panel p-6">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-indigo-50/50 rounded-xl text-[var(--accent-primary)] border border-indigo-100/20"><Shield size={20} /></div>
        ...
        <section className="glass-panel p-6">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-indigo-50/50 rounded-xl text-[var(--accent-primary)] border border-indigo-100/20"><Network size={20} /></div>
        ...
        <section className="glass-panel p-6 md:col-span-2">
          <div className="flex items-center gap-3 mb-8">
            <div className="p-3 bg-indigo-50/50 rounded-xl text-[var(--accent-primary)] border border-indigo-100/20"><LayoutGrid size={20} /></div>
          ...
          <div className="space-y-10">
            ...
            {/* Sidebar & New Tab Shortcuts Management */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Sidebar */}
              <div className="glass-panel bg-white/40 rounded-2xl p-5">
                <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-5 uppercase tracking-wider pl-2">Sidebar Apps</h3>
                <div className="space-y-4 mb-6">
                  {settings.sidebarApps.map(app => (
                    <div key={app.id} className="flex items-center justify-between glass-panel p-3">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-white/60 shadow-sm text-${app.color || 'indigo-500'}`}><LayoutGrid size={18}/></div>
                        <div>
                      ...
                      <button onClick={() => removeShortcut('sidebarApps', app.id)} className="text-red-400 hover:text-red-500 p-2 glass-btn rounded-full mr-1"><Trash2 size={16}/></button>
                    </div>
                  ))}
                </div>
                <button onClick={() => {
                   const name = prompt('App Name:');
                   const url = prompt('App URL (e.g. https://example.com):');
                   if (name && url) {
                     onToggle('sidebarApps', [...settings.sidebarApps, { id: Date.now().toString(), name, url, icon: 'Globe', color: 'indigo-500' }]);
                   }
                }} className="w-full py-4 rounded-xl glass-btn text-[var(--text-secondary)] font-medium flex items-center justify-center gap-2">
                  <Plus size={18}/> Add Sidebar App
                </button>
              </div>
              ...
              <div className="glass-panel bg-white/40 rounded-2xl p-5">
                ...
                <div className="space-y-4 mb-6">
                  {settings.newTabShortcuts.map(app => (
                    <div key={app.id} className="flex items-center justify-between glass-panel p-3">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-white/60 shadow-sm text-[var(--text-secondary)]"><Globe size={18}/></div>
                        <div>
                          <div className="text-sm font-medium text-[var(--text-primary)]">{app.name}</div>
                          <div className="text-xs text-[var(--text-secondary)] truncate max-w-[150px] mt-1">{app.url}</div>
                        </div>
                ...
                <button onClick={() => {
                   ...
                   if (name && url) {
                     onToggle('newTabShortcuts', [...settings.newTabShortcuts, { id: Date.now().toString(), name, url, icon: 'Globe', color: 'slate-800' }]);
                   }
                }} className="w-full py-4 rounded-xl glass-btn text-[var(--text-secondary)] font-medium flex items-center justify-center gap-2">
        ...
        <section className="glass-panel p-6 md:col-span-2">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-indigo-50/50 rounded-xl text-[var(--accent-primary)] border border-indigo-100/20"><Settings2 size={20} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="px-6 py-3 rounded-xl text-sm font-medium glass-btn text-[var(--text-primary)] hover:bg-white/60 transition-all flex items-center gap-2"
                  >
                    <DownloadCloud size={16} /> Export
                  </button>
                  <label className="px-6 py-3 rounded-xl text-sm font-medium glass-btn text-[var(--text-primary)] hover:bg-white/60 transition-all flex items-center gap-2 cursor-pointer">
            ...
            <div className="glass-panel bg-white/40 rounded-2xl p-6 flex flex-col justify-center items-center text-center">
